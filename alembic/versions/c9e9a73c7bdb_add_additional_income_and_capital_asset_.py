...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "end_date IS NULL OR end_date >= start_date", name="check_valid_date_range"
        ),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            "fixed_rate IS NULL OR fixed_rate >= 0",
            name="check_non_negative_fixed_rate",
        ),
        sa.CheckConstraint(
            "tax_rate IS NULL OR (tax_rate >= 0 AND tax_rate <= 1)",
            name="check_valid_tax_rate",
        ),
    ...
    op.create_index(
        op.f("ix_additional_income_id"), "additional_income", ["id"], unique=False
    )
    op.create_index(
        op.f("ix_additional_income_client_id"),
        "additional_income",
        ["client_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "annual_return_rate", sa.Numeric(precision=5, scale=4), nullable=False
        ),
    ...
    op.create_index(
        op.f("ix_capital_assets_id"), "capital_assets", ["id"], unique=False
    )
    op.create_index(
        op.f("ix_capital_assets_client_id"),
        "capital_assets",
        ["client_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_additional_income_client_id"), table_name="additional_income"
    )
