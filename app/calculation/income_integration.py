...
def integrate_additional_incomes_with_scenario(
    ...
    logger.debug(
        f"Generating additional income cashflow from {start_date} to {end_date}"
    )
    ...
    for scenario_item in scenario_cashflow:
        ...
        if scenario_date in income_by_date:
            ...
            integrated_item["additional_income_gross"] = float(
                income_item["gross_amount"]
            )
            integrated_item["additional_income_tax"] = float(
                income_item["tax_amount"]
            )  # מס מוצג (כולל מס קבוע)
            integrated_item["additional_income_tax_for_total"] = float(
                income_item.get("tax_amount_for_total", income_item["tax_amount"])
            )  # מס לסה"כ (ללא מס קבוע)
            integrated_item["additional_income_net"] = float(income_item["net_amount"])

            # Update total inflow and net
            integrated_item["inflow"] = float(
                Decimal(str(integrated_item["inflow"])) + income_item["net_amount"]
            )
            integrated_item["net"] = float(
                Decimal(str(integrated_item["net"])) + income_item["net_amount"]
            )
        else:
    ...
    logger.debug(
        f"Integrated additional incomes into {len(integrated_cashflow)} cashflow items"
    )
...
def integrate_capital_assets_with_scenario(
    ...
    for scenario_item in scenario_cashflow:
        ...
        if scenario_date in asset_by_date:
            ...
            # Update total inflow and net
            integrated_item["inflow"] = float(
                Decimal(str(integrated_item["inflow"])) + asset_item["net_return"]
            )
            integrated_item["net"] = float(
                Decimal(str(integrated_item["net"])) + asset_item["net_return"]
            )
        else:
    ...
    logger.debug(
        f"Integrated capital assets into {len(integrated_cashflow)} cashflow items"
    )
    return integrated_cashflow
...
def integrate_all_incomes_with_scenario(
    ...
    logger.debug(
        f"Fully integrated cashflow with {len(fully_integrated_cashflow)} items"
    )
