...
def calculate_total_monthly_pension(
...
def generate_combined_pension_cashflow(
    db: Session, client_id: int, months: int = 12, reference_date: Optional[date] = None
) -> List[Dict[str, Any]]:
    ...
    for fund in funds:
        fund_cashflow = PensionEngine._project_pension_cashflow(
            fund, months, params, start_date
        )
...
def integrate_pension_funds_with_scenario(
    db: Session,
    client_id: int,
    ...
    pension_cashflow = generate_combined_pension_cashflow(
        db, client_id, months, reference_date
    )
