...
def calculate_grant_tax(
    ...
    if service_years and service_years > 0:
        ...
        if amount_dec <= exempt_cap:
            calculation["grant_exempt"] = float(
                amount_dec.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            )
            calculation["grant_taxable"] = 0.0
            calculation["tax_due"] = 0.0
        else:
            calculation["grant_exempt"] = float(
                exempt_cap.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            )
            taxable = amount_dec - exempt_cap
            calculation["grant_taxable"] = float(
                taxable.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            )
            calculation["tax_due"] = float(
                (taxable * Decimal(str(tax_rate))).quantize(
        ...
        calculation["tax_due"] = float(
            (amount_dec * Decimal(str(tax_rate))).quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            )
        )
