...
FIXATION_ALLOW_JSON_FALLBACK = (
    os.getenv("FIXATION_ALLOW_JSON_FALLBACK", "true").lower() == "true"
)
...
def _fill_form(template: Path, output_dir: Path, file_name: str, fields: dict) -> Path:
    ...
    if FIXATION_ALLOW_JSON_FALLBACK:
        ...
        raise HTTPException(
            status_code=503,
            detail={
                "error": "׳¡׳₪׳¨׳™׳•׳× PDF ׳׳™׳ ׳ ׳–׳׳™׳ ׳•׳× ׳•-fallback ׳-JSON ׳׳ ׳•׳˜׳¨׳"
            },
        )


def fill_161d_form(
    db: Session, client_id: int, template_path: Path, output_dir: Path
) -> Path:
    """
    ׳׳׳׳ ׳˜׳•׳₪׳¡ 161׳“.
    """
    client = (
        db.query(Client).filter(Client.id == client_id, Client.is_active == True).one()
    )
    return _fill_form(
        template_path, output_dir, f"161d_{client.id}", build_161d_fields(client)
    )


def fill_grants_appendix(
    db: Session, client_id: int, template_path: Path, output_dir: Path
) -> Path:
    """
    ׳׳׳׳ ׳ ׳¡׳₪׳— ׳׳¢׳ ׳§׳™׳.
    ...
    return _fill_form(
        template_path,
        output_dir,
        f"grants_appendix_{client.id}",
        build_grants_fields(client),
    )


def fill_commutations_appendix(
    db: Session, client_id: int, template_path: Path, output_dir: Path
) -> Path:
    """
    ׳׳׳׳ ׳ ׳¡׳₪׳— ׳”׳™׳•׳•׳ ׳™׳.
    ...
    return _fill_form(
        template_path,
        output_dir,
        f"commutations_appendix_{client.id}",
        build_commutations_fields(client),
    )
