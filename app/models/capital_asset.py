...
class CapitalAsset(Base):
    ...
    rental_income = Column(
        Numeric(15, 2), nullable=True
    )  # הכנסה משכירות (לתאימות לאחור)
    monthly_rental_income = Column(
        Numeric(15, 2), nullable=True
    )  # הכנסה חודשית משכירות (לתאימות לאחור)
    ...
    original_principal = Column(
        Numeric(15, 2), nullable=True
    )  # צבירה מקורית - למס רווח הון
    ...
    __table_args__ = (
        CheckConstraint(
            "current_value >= 0", name="check_positive_value"
        ),  # ✅ מאפשר 0 להיוונים
