...
class Client(Base):
    ...
    # Relationships
    fixation_results = relationship(
        "FixationResult", back_populates="client", cascade="all, delete-orphan"
    )
    pension_funds = relationship(
        "PensionFund", back_populates="client", cascade="all, delete-orphan"
    )
    current_employers = relationship(
        "CurrentEmployer", back_populates="client", cascade="all, delete-orphan"
    )
    grants = relationship(
        "Grant", back_populates="client", cascade="all, delete-orphan"
    )
    additional_incomes = relationship(
        "AdditionalIncome", back_populates="client", cascade="all, delete-orphan"
    )
    capital_assets = relationship(
        "CapitalAsset", back_populates="client", cascade="all, delete-orphan"
    )
    scenarios = relationship(
        "Scenario", back_populates="client", cascade="all, delete-orphan"
    )
    ...
    def get_age(self, reference_date: date = None) -> int:
        ...
        if (ref_date.month, ref_date.day) < (
...
def _client_fill_id_number_raw_before_insert(mapper, connection, target):
    ...
    if not getattr(target, "id_number_raw", None) and getattr(
        target, "id_number", None
    ):
