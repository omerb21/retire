...
class CurrentEmployer(Base):
    ...
    )  # JSON object for various grants
    tax_withheld = Column(Float, nullable=True)
    grant_installments = Column(
        JSON, nullable=True, default=list
    )  # JSON array of installment details

    # Continuity and pension
    active_continuity = Column(
        SQLEnum(ActiveContinuityType), nullable=True, default=ActiveContinuityType.none
    )
    continuity_years = Column(Float, nullable=False, default=0.0)
    pre_retirement_pension = Column(Float, nullable=True)
    existing_deductions = Column(
        JSON, nullable=True, default=dict
    )  # JSON object for deduction details
    ...
    grants = relationship(
        "EmployerGrant", back_populates="employer", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        return {
            ...
            "active_continuity": (
                self.active_continuity.value if self.active_continuity else None
            ),
