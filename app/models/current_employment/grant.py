...
class EmployerGrant(Base):
    ...
    employer_id = Column(
        Integer, ForeignKey("current_employer.id", ondelete="CASCADE"), nullable=False
    )
    ...
    )  # סוג המוצר (קרן פנסיה / ביטוח מנהלים / קופת גמל)
    ...
    indexed_amount = Column(
        Float, nullable=True
    )  # Amount after indexing/CPI adjustment
    ...
    def to_dict(self):
        return {
            ...
            "plan_start_date": (
                self.plan_start_date.isoformat() if self.plan_start_date else None
            ),
