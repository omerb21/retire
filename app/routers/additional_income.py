...
router = APIRouter(
    prefix="/clients/{client_id}/additional-incomes", tags=["additional-incomes"]
)


@router.post(
    "", response_model=AdditionalIncomeResponse, status_code=status.HTTP_201_CREATED
)
@router.post(
    "/", response_model=AdditionalIncomeResponse, status_code=status.HTTP_201_CREATED
)
def create_additional_income(
    client_id: int, income_data: AdditionalIncomeCreate, db: Session = Depends(get_db)
):
    ...
    db_income = AdditionalIncome(client_id=client_id, **income_data.model_dump())
...
@router.get("/", response_model=List[AdditionalIncomeResponse])
def get_additional_incomes(client_id: int, db: Session = Depends(get_db)):
    ...
    incomes = (
        db.query(AdditionalIncome).filter(AdditionalIncome.client_id == client_id).all()
    )
...
def get_additional_income(
    client_id: int, income_id: int, db: Session = Depends(get_db)
):
    """Get a specific additional income source."""
    income = (
        db.query(AdditionalIncome)
        .filter(
            AdditionalIncome.id == income_id, AdditionalIncome.client_id == client_id
        )
        .first()
    )
