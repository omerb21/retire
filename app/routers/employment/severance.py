...
def calculate_severance(
    request_data: SeveranceCalculationRequest, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        def parse_date(value: str | None):
            ...
            for fmt in ("%Y-%m-%d", "%d/%m/%Y"):
                try:
                    ...
                    continue
            raise ValueError(
                f"time data '{value}' does not match supported date formats"
            )

        # Convert dates
        start_date = parse_date(request_data.start_date)
        end_date = (
            parse_date(request_data.end_date) if request_data.end_date else date.today()
        )
        ...
        raise HTTPException(
            status_code=400, detail={"error": f"שגיאה בחישוב פיצויים: {str(e)}"}
        )
