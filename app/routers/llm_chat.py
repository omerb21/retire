...
async def pension_chat(request: ChatRequest, db: Session = Depends(get_db), http_request: Request = None) -> ChatResponse:
    ...
    try:
        ...
        try:
            ...
            log_trace_event(event_type="assistant_output", payload=_ao_payload, client_id=request.client_id, endpoint=endpoint)
            _eyes_emit("assistant_output", _ao_payload, client_id=request.client_id, endpoint=endpoint)
        except Exception:
            pass
