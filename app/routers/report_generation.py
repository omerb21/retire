...
async def generate_pdf_report(
    ...
    **Example Usage:**
    ...
    try:
        logger.info(
            f"PDF report request: client_id={client_id}, scenario_id={scenario_id}, range={request.from_}-{request.to}"
        )

        # Generate PDF using the adapter with detailed error handling
        try:
            pdf_bytes = ReportServiceAdapter.generate_pdf_report(
                db=db, client_id=client_id, scenario_id=scenario_id, request=request
            )
            ...
            if (
                ...
                logger.error(
                    f"Generated invalid PDF format: {len(pdf_bytes)} bytes, starts with: {pdf_bytes[:20]}"
                )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Internal server error during PDF generation: {type(e).__name__}",
            )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error in PDF endpoint"
        )
