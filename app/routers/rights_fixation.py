...
async def calculate_rights_fixation(
    ...
    """
    try:
        logger.debug("Rights fixation router called with payload: %s", client_data)
...
@router.post("/save")
async def save_rights_fixation(data: Dict[str, Any], db: Session = Depends(get_db)):
    """
...
async def get_saved_fixation(client_id: int, db: Session = Depends(get_db)):
    """
...
async def delete_fixation(client_id: int, db: Session = Depends(get_db)):
