...
async def calculate_tax(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"שגיאה בחישוב מס: {str(e)}")
...
async def analyze_tax_comprehensive(
    ...
    projection_years: int = Query(
        default=5, ge=1, le=20, description="מספר שנים לתחזית"
    ),
    calculator: TaxCalculator = Depends(get_tax_calculator),
) -> ComprehensiveTaxAnalysis:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"שגיאה בניתוח מס מקיף: {str(e)}")
...
async def get_tax_brackets(year: int):
    ...
    try:
        ...
        return {"year": year, "brackets": brackets, "currency": "ILS"}

    except Exception as e:
        logger.error(f"שגיאה בקבלת מדרגות מס לשנת {year}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"שגיאה בקבלת מדרגות מס: {str(e)}")
...
async def get_available_tax_credits(year: int):
    ...
    try:
        ...
        return {"year": year, "credits": credits, "currency": "ILS"}

    except Exception as e:
        logger.error(f"שגיאה בקבלת נקודות זיכוי לשנת {year}: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"שגיאה בקבלת נקודות זיכוי: {str(e)}"
        )


@router.post("/simulate")
async def simulate_tax_scenarios(
    base_input: TaxCalculationInput, scenarios: List[dict]
):
    """
    ...
    try:
        ...
        results["base"] = {"description": "תרחיש בסיס", "result": base_result}
        ...
        return {
            ...
            "comparison_summary": {
                "best_scenario": min(
                    results.items(), key=lambda x: x[1]["result"].net_tax
                )[0],
                "max_savings": max(
        ...
        raise HTTPException(
            status_code=500, detail=f"שגיאה בסימולציית תרחישים: {str(e)}"
        )
...
async def health_check():
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"שירות חישוב המס אינו זמין: {str(e)}"
        )
