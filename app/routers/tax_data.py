...
def get_severance_cap(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching severance cap: {str(e)}"
        )
...
def get_severance_caps():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching severance caps: {str(e)}"
        )
...
def update_severance_caps(caps: List[Dict]):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to update severance caps"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating severance caps: {str(e)}"
        )
...
def get_cpi_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching CPI data: {str(e)}"
        )
...
def get_tax_brackets(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching tax brackets: {str(e)}"
        )
...
def get_indexation_factor(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating indexation factor: {str(e)}"
        )
...
def calculate_severance_exemption(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating severance exemption: {str(e)}"
        )
...
def update_tax_data_cache():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating tax data cache: {str(e)}"
        )


@router.get("/summary")
def get_tax_data_summary(
    year: Optional[int] = Query(None, description="Year (default: current year)")
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching tax data summary: {str(e)}"
        )
