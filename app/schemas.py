...
class ClientBase(BaseSchema):
    id_number: str = Field(
        ..., min_length=9, max_length=9, description="Israeli ID number"
    )
    ...
    pension_start_date: Optional[date] = Field(
        None, description="תאריך תחילת קבלת פנסיה"
    )
    spouse_income: Optional[float] = Field(None, ge=0, description="הכנסת בן/בת זוג")
    immigration_date: Optional[date] = Field(
        None, description="תאריך עלייה (לעולים חדשים)"
    )
    military_discharge_date: Optional[date] = Field(
        None, description='תאריך שחרור מצה"ל'
    )
...
class ClientUpdate(BaseSchema):
    ...
    pension_contributions: Optional[float] = Field(
        None, ge=0, description="הפרשות לפנסיה"
    )
    study_fund_contributions: Optional[float] = Field(
        None, ge=0, description="הפרשות לקרן השתלמות"
    )
    ...
    # Tax credit points - נקודות זיכוי
    tax_credit_points: Optional[float] = Field(
        None, ge=0, description="נקודות זיכוי במס (ידני)"
    )
    pension_start_date: Optional[date] = Field(
        None, description="תאריך תחילת קבלת פנסיה"
    )
