...
class AdditionalIncomeBase(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=255, description="Description of the income source"
    )
    ...
    tax_rate: Optional[Decimal] = Field(
        None, ge=0, le=99, description="Tax rate for fixed rate tax (0-99%)"
    )
...
class AdditionalIncomeCashflowItem(BaseModel):
    ...
    include_in_total_tax: bool = Field(
        True, description='האם לכלול את המס בסה"כ המס בתזרים'
    )
