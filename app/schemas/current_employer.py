...
class CurrentEmployerBase(BaseModel):
    ...
    employer_id_number: Optional[str] = Field(
        None, description="מספר זהות/ח.פ. של המעסיק"
    )
    start_date: date = Field(..., description="תאריך תחילת עבודה")
    end_date: Optional[date] = Field(
        None, description="תאריך סיום עבודה (null אם עדיין עובד)"
    )
    non_continuous_periods: Optional[List[Dict[str, Any]]] = Field(
        default_factory=list, description="תקופות לא רציפות"
    )
    ...
    monthly_salary: Optional[float] = Field(
        None, description="שכר חודשי (ממופה ל-last_salary)"
    )
    severance_balance: Optional[float] = Field(
        None, description="יתרת פיצויים (ממופה ל-severance_accrued)"
    )
    other_grants: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="מענקים אחרים"
    )
    tax_withheld: Optional[float] = Field(None, description="מס שנוכה")
    grant_installments: Optional[List[Dict[str, Any]]] = Field(
        default_factory=list, description="תשלומי מענק"
    )
    active_continuity: Optional[ActiveContinuityType] = Field(
        None, description="רציפות פעילה"
    )
    continuity_years: Optional[float] = Field(0.0, description="שנות רציפות")
    pre_retirement_pension: Optional[float] = Field(
        None, description="פנסיה לפני פרישה"
    )
    existing_deductions: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="ניכויים קיימים"
    )
...
class TerminationDecisionBase(BaseModel):
    ...
    exempt_choice: str = Field(
        ...,
        description="בחירה לחלק הפטור: redeem_with_exemption/redeem_no_exemption/annuity",
    )
    taxable_choice: str = Field(
        ...,
        description="בחירה לחלק החייב: redeem_no_exemption/annuity/split (פריסת מס אוטומטית ב-redeem_no_exemption)",
    )
    taxable_annuity_amount: Optional[float] = Field(
        None,
        description="D4.1: סכום מדויק מתוך היתרה החייבת לרצף קצבה (כאשר taxable_choice=split)",
    )
    taxable_capital_amount: Optional[float] = Field(
        None,
        description="D4.1: סכום מדויק מתוך היתרה החייבת למענק הוני (כאשר taxable_choice=split)",
    )
    tax_spread_years: Optional[int] = Field(None, description="מספר שנות פריסת מס")
    max_spread_years: Optional[int] = Field(
        None, description="מקסימום שנות פריסה מותרות"
    )
    confirmed: Optional[bool] = Field(
        False, description="האם העזיבה אושרה והנתונים הוקפאו"
    )
    source_accounts: Optional[str] = Field(
        None, description="רשימת שמות התכניות שמהן נלקחו הפיצויים (JSON)"
    )
    plan_details: Optional[str] = Field(
        None, description="פרטי תכניות מלאים: שם, תאריך התחלה, סכום (JSON)"
    )
...
class TerminationDecisionOut(TerminationDecisionBase):
    ...
    created_capital_asset_id: Optional[int] = Field(
        None, description="ID של נכס הון שנוצר"
    )

    warning_code: Optional[str] = Field(
        None, description="קוד אזהרה כאשר הנתון חסר והמערכת משתמשת בהערכה"
    )
    used_fallback_expected_severance: Optional[bool] = Field(
        None, description="האם חושב פיצוי צפוי כי severance_accrued חסר"
    )
...
class SeveranceCalculationRequest(BaseModel):
    ...
    start_date: Optional[str] = Field(
        None, description="תאריך התחלת עבודה (YYYY-MM-DD)"
    )
