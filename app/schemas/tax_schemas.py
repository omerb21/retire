...
class TaxCreditInput(BaseModel):
    ...
    amount: Optional[float] = Field(
        None, description="סכום הזיכוי (אם שונה מברירת המחדל)"
    )
...
class PersonalDetails(BaseModel):
    ...
    disability_percentage: Optional[int] = Field(
        None, ge=0, le=100, description="אחוז נכות"
    )
...
class TaxCalculationInput(BaseModel):
    ...
    tax_year: int = Field(
        default_factory=lambda: date.today().year, description="שנת מס"
    )
    personal_details: PersonalDetails = Field(..., description="פרטים אישיים")
    income_sources: List[IncomeSource] = Field(
        default_factory=list, description="מקורות הכנסה"
    )
    additional_tax_credits: List[TaxCreditInput] = Field(
        default_factory=list, description="זיכויים נוספים"
    )
    ...
    # פטורים וקצבאות
    exempt_pension_amount: float = Field(
        0, ge=0, description="קצבה פטורה ממס (מקיבוע זכויות)"
    )
    pension_months_in_year: int = Field(
        12, ge=1, le=12, description="מספר חודשי קצבה בשנה"
    )
...
class TaxCalculationResult(BaseModel):
    ...
    special_taxes: Dict[str, float] = Field(
        default_factory=dict, description="מסים מיוחדים (שכירות, רווח הון וכו')"
    )
    total_tax: float = Field(..., description="סך המסים")

    # זיכויים
    tax_credits_amount: float = Field(..., description="סך נקודות הזיכוי")
    applied_credits: List[TaxCreditInput] = Field(
        default_factory=list, description="זיכויים שהוחלו"
    )
    ...
    tax_breakdown: List[TaxBreakdown] = Field(
        default_factory=list, description="פירוט מס לפי מדרגות"
    )
    income_breakdown: List[IncomeTypeBreakdown] = Field(
        default_factory=list, description="פירוט הכנסות לפי סוג"
    )

    # מטא-דאטה
    calculation_date: datetime = Field(
        default_factory=datetime.now, description="תאריך החישוב"
    )
...
class AnnualTaxProjection(BaseModel):
    ...
    monthly_projections: List[MonthlyTaxProjection] = Field(
        default_factory=list, description="תחזית חודשית"
    )
...
class ComprehensiveTaxAnalysis(BaseModel):
    ...
    annual_projections: List[AnnualTaxProjection] = Field(
        default_factory=list, description="תחזיות שנתיות"
    )
    optimization_suggestions: List[TaxOptimizationSuggestion] = Field(
        default_factory=list, description="הצעות אופטימיזציה"
    )
    comparison_scenarios: Dict[str, TaxCalculationResult] = Field(
        default_factory=dict, description="תרחישי השוואה"
    )
