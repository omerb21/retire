...
class AdditionalIncomeService:
    ...
    def apply_indexation(
        ...
        if income.indexation_method == IndexationMethod.FIXED:
            ...
            raise ValueError(
                f"Unsupported indexation method: {income.indexation_method}"
            )
    ...
    def project_cashflow(
        ...
        logger.debug(
            f"Projecting cashflow for income {income.id} from {start_date} to {end_date}"
        )
        ...
        logger.info(
            f"🔵 Income frequency={income.frequency}, interval={payment_interval}, amount={payment_amount}"
        )
        ...
        while current_date <= actual_end_date:
            ...
            if should_pay:
                ...
                # Calculate tax
                tax_amount, include_in_total = self.calculate_tax(
                    indexed_amount, income, client, current_date
                )
    ...
    def generate_combined_cashflow(
        ...
        logger.debug(
            f"Generating combined additional income cashflow for client {client_id}"
        )
        ...
        for item in all_cashflow_items:
            ...
            if date_key not in aggregated_cashflow:
                aggregated_cashflow[date_key] = {
                    ...
                    "tax_amount": Decimal("0"),
                    "tax_amount_for_total": Decimal(
                        "0"
                    ),  # רק מסים שצריכים להיכלל בסה"כ
                    "net_amount": Decimal("0"),
                }
    ...
    def _calculate_cpi_factor(
        self, cpi_series: Dict[int, Decimal], start_date: date, end_date: date
    ) -> Decimal:
        ...
        if (
            start_year == end_year
            or start_year not in cpi_series
            or end_year not in cpi_series
        ):
            return Decimal("1")
