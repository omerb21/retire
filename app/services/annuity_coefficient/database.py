...
def get_generation_coefficient(db, generation_code: str, age: int, sex: str) -> dict:
    ...
    if result:
        ...
        return {
            ...
            "source_keys": {"generation_code": generation_code, "age": age, "sex": sex},
            "target_year": None,
            "guarantee_months": guarantee,
            "notes": notes or "",
        }

    logger.warning(f"[מקדם קצבה] לא נמצאה שורה עבור דור={generation_code}, גיל={age}")
    return get_default_coefficient()
...
def get_pension_fund_coefficient_from_db(
    ...
    if result:
        ...
        return {"factor": factor, "fund_name": fund_name, "notes": notes}
