...
def calculate_service_years(
    ...
    non_continuous_periods: Optional[List[Dict]] = None,
) -> Dict[str, Any]:
    """
...
def apply_indexation(
    ...
    return {
        ...
        "increase_percentage": (
            float((indexed_amount - base_amount) / base_amount * 100)
            if base_amount > 0
            else 0
        ),
    }
...
def calculate_severance_grant(
    salary: Decimal, service_years: Decimal, tax_params: Optional[Dict] = None
) -> Dict[str, Any]:
    """
...
def generate_cashflow(
    client_id: int, scenario_params: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
def calculate_tax_impact_for_client(
    client_data: Dict[str, Any],
    annual_income: float = 0,
    pension_income: float = 0,
    additional_incomes: List[Dict] = None,
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if additional_incomes:
            for income in additional_incomes:
                ...
                amount = income.get("annual_amount", 0) or (
                    income.get("monthly_amount", 0) * 12
                )
...
def calculate_retirement_tax_projection(
    client_data: Dict[str, Any],
    ...
    try:
        ...
        for year_offset in range(projection_years):
            ...
            if pension_funds:
                for fund in pension_funds:
                    monthly_amount = fund.get("computed_monthly_amount", 0) or fund.get(
                        "monthly_amount", 0
                    )
                    # הצמדה שנתית של 2.5%
            ...
            tax_impact = calculate_tax_impact_for_client(
                client_data=client_data, pension_income=total_pension_income
            )
                ...
                projections.append(
                    {
                        ...
                        "effective_tax_rate": tax_impact["tax_calculation"][
                            "effective_tax_rate"
                        ],
                        "monthly_net": tax_impact["monthly_impact"][
                            "net_monthly_income"
                        ],
                    }
                )
        ...
        return {
            ...
            "summary": {
                "average_annual_tax": (
                    sum(p["total_tax"] for p in projections) / len(projections)
                    if projections
                    else 0
                ),
                "average_net_income": (
                    sum(p["net_income"] for p in projections) / len(projections)
                    if projections
                    else 0
                ),
