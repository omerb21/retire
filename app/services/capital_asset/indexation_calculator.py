...
class IndexationCalculator(BaseCalculator):
    ...
    def _calculate_fixed_indexation(
        self, amount: Decimal, years: int, rate: Optional[Decimal]
    ) -> Decimal:
        """
    ...
    def _calculate_cpi_indexation(
        self, amount: Decimal, start_date: date, end_date: date
    ) -> Decimal:
        """
    ...
    def _get_cpi_factor(self, start_date: date, end_date: date) -> Decimal:
        ...
        if (
            start_year == end_year
            or start_year not in self.cpi_series
            or end_year not in self.cpi_series
        ):
            return Decimal("1")
