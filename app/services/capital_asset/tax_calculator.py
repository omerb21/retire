...
class TaxCalculator(BaseCalculator):
    ...
    def __init__(
        self, tax_brackets: Optional[List[Tuple[Optional[Decimal], Decimal]]] = None
    ):
        """
        אתחול מחשבון המס.
    ...
    def calculate(
        ...
        if tax_treatment == TaxTreatment.EXEMPT:
            ...
            return self._calculate_spread_tax(
                gross_amount, spread_years, annual_regular_income
            )
        else:
    ...
    def validate_inputs(
        ...
        if tax_treatment == TaxTreatment.TAX_SPREAD and (
            spread_years is None or spread_years <= 0
        ):
    ...
    def _calculate_fixed_rate(
        self, amount: Decimal, rate: Optional[Decimal]
    ) -> Dict[str, Any]:
        """
        חשב מס בשיעור קבוע.
