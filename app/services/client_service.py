...
def validate_employment_flags(
    self_employed: bool, current_employer_exists: bool
) -> bool:
    """
...
def validate_client_data(data: Dict[str, Any]) -> Dict[str, str]:
    ...
    if "self_employed" in data and "current_employer_exists" in data:
        if not validate_employment_flags(
            data["self_employed"], data["current_employer_exists"]
        ):
