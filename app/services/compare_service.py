...
def compare_scenarios(
    ...
    if frequency != "monthly":
        log_calc(
            "compare_scenarios_error", payload, None, {"error": "Invalid frequency"}
        )
    ...
    for sid in scenario_ids:
        ...
        monthly = generate_cashflow(
            db_session, client_id, sid, from_yyyymm, to_yyyymm, case_id=case_id
        )
    ...
    result = {
        ...
        "meta": {
            ...
            "to": to_yyyymm,
            "frequency": frequency,
            "case": {"id": case_id, "name": case_name, "reasons": case_reasons},
        },
    }
...
def _validate_scenario_results_new(result: Dict[str, Any]):
    ...
    for scenario in result.get("scenarios", []):
        ...
        if month_count < 12:
            warnings.append(
                f"Scenario {scenario_id}: Only {month_count}/12 months available"
            )
        ...
        for year, totals in yearly_data.items():
            ...
            if net < -1000000:  # Less than -1M NIS per year
                warnings.append(
                    f"Scenario {scenario_id}, Year {year}: Extremely negative net cashflow ({net:,.0f})"
                )
            ...
            if inflow > 50000000:  # More than 50M NIS per year
                warnings.append(
                    f"Scenario {scenario_id}, Year {year}: Unrealistic inflow amount ({inflow:,.0f})"
                )
            ...
            if add_income < -100000:  # Less than -100K (some negative is OK for taxes)
                warnings.append(
                    f"Scenario {scenario_id}, Year {year}: Suspicious additional income ({add_income:,.0f})"
                )
            ...
            if abs(computed_net - net) > 0.02:  # Allow small rounding difference
                warnings.append(
                    f"Scenario {scenario_id}, Year {year}: Net inconsistency. Reported: {net}, Computed: {computed_net}"
                )
