...
class GrantService:
    @staticmethod
    def create_grant(
        db: Session, client_id: int, grant_data: GrantCreate
    ) -> Tuple[Grant, Dict[str, float]]:
        """
        ...
        grant.grant_indexed_amount = calculation.get(
            "grant_exempt", 0
        ) + calculation.get("grant_taxable", 0)
    ...
    def calculate_tax_implications(
        grant: Grant, service_years: float = None
    ) -> Dict[str, float]:
        """
        ...
        """
        # Default values
        calculation = {"grant_exempt": 0.0, "grant_taxable": 0.0, "tax_due": 0.0}
        ...
        if service_years and service_years > 0:
            ...
            tax_calc = calculate_grant_tax(
                amount=grant.grant_amount, service_years=service_years
            )
            ...
            tax_calc = calculate_grant_tax(
                amount=grant.grant_amount, service_years=None
            )
            calculation["grant_exempt"] = tax_calc["grant_exempt"]
            calculation["grant_taxable"] = tax_calc["grant_taxable"]
            calculation["tax_due"] = tax_calc["tax_due"]
    ...
    @staticmethod
    def update_grant(
        db: Session, grant_id: int, grant_data: GrantUpdate
    ) -> Tuple[Grant, Dict[str, float]]:
        """
        ...
        calculation = GrantService.calculate_tax_implications(
            grant, grant_data.service_years
        )
