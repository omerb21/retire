...
class IndexationService:
    ...
    def calculate_adjusted_amount(
        ...
        try:
            ...
            if not answer:
                logger.warning(
                    f"אזהרה: API ללא answer עבור {end_work_date} | תשובה: {data}"
                )
                return None
            ...
            if to_value is None:
                logger.warning(
                    f"אזהרה: אין to_value עבור {end_work_date} | תשובה: {data}"
                )
                return None
    ...
    def index_grant(
        ...
        if elig_date:
            ...
            return IndexationService.calculate_adjusted_amount(
                amount, end_work_date, elig_date_obj
            )
        else:
        ...
        try:
            ...
            if birth_date and gender:
                try:
                    ...
                    logger.info(
                        f"[יחסי מענק] גיל פרישה מחושב: {retirement_date} (מגדר: {gender})"
                    )
                except Exception as e:
            ...
            if retirement_date and end_date > retirement_date:
                ...
                logger.info(
                    f"[יחסי מענק] הגבלת תאריך סיום מ-{end_date} ל-{effective_end_date} (גיל פרישה)"
                )
            ...
            if total_days <= 0:
                logger.info(
                    f"[יחסי מענק] אין ימי עבודה רלוונטיים (עבודה לאחר גיל פרישה)"
                )
                return 0.0
            ...
            logger.info(
                f"[יחסי מענק] תאריך ייחוס={today}, התחלה={start_date}, "
            ...
            logger.error(
                f"[שגיאה ביחס מענק] התחלה={start_date}, סיום={end_date}, שגיאה: {str(e)}"
            )
            return 0.0

    @staticmethod
    def calculate_eligibility_age(
        birth_date: date, gender: str, pension_start_date: date
    ) -> date:
        """
        חישוב תאריך זכאות לפי גיל פרישה
        """
        from app.services.retirement_age_service import (
            calc_eligibility_date,
            get_retirement_date,
        )
    ...
    @staticmethod
    def calculate_exact_grant_value(
        grant_data: Dict[str, Any], eligibility_date: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "calculation_details": {
                    ...
                    or datetime.today().date().isoformat(),
