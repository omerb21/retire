...
class CommutationToolsMixin:
    def calculate_pension_commutation(
        ...
        if (ret_date.month, ret_date.day) < (
        ...
        if comparison.get("recommendation") == "lump_sum":
            explanation_lines.append(
                f"✅ **המלצה:** ההיוון משתלם כלכלית (הפרש: {comparison['difference']:,.0f} ₪ לטובתך)"
            )
        else:
            explanation_lines.append(
                f"⚠️ **המלצה:** הקצבה משתלמת יותר כלכלית (הפרש: {abs(comparison['difference']):,.0f} ₪)"
            )
