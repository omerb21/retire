            ...
            if include_charts and scenarios:
                ...
                if scenarios[0].cashflow_projection:
                    try:
                        ...
                        cashflow_data = (
                            json.loads(scenarios[0].cashflow_projection)
                            if isinstance(scenarios[0].cashflow_projection, str)
                            else scenarios[0].cashflow_projection
                        )
    ...
    def compose_pdf(
        ...
        story.append(
            Paragraph(
                "פרטי הלקוח",
                ParagraphStyle(
                    "SectionTitle", parent=hebrew_style, fontSize=14, spaceAfter=10
                ),
            )
        )
        ...
        if summary["scenarios_summary"]:
            ...
            for scenario_sum in summary["scenarios_summary"]:
                ...
                story.append(
                    Paragraph(
                        f"תאריך יצירה: {scenario_sum['created_at']}", hebrew_style
                    )
                )
        ...
        if chart_cashflow:
            ...
            cashflow_img = Image(
                io.BytesIO(chart_cashflow), width=6 * inch, height=3 * inch
            )
        ...
        if chart_compare:
            ...
            compare_img = Image(
                io.BytesIO(chart_compare), width=6 * inch, height=3 * inch
            )
        ...
        story.append(
            Paragraph(
                ...
                ParagraphStyle(
                    "Footer", parent=hebrew_style, fontSize=8, alignment=TA_CENTER
                ),
            )
        )
