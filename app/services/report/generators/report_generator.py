...
def generate_report_pdf(
    ...
    _logger.info(
        f"Starting PDF report generation for client={client_id}, scenario={scenario_id}, range={request.from_}-{request.to}"
    )
    ...
    try:
        ...
        for sid in scenario_ids:
            try:
                ...
                _logger.info(
                    f"Generated {len(cashflow_rows)} cashflow rows for scenario_id={sid}"
                )
                cashflow_data.extend(cashflow_rows)
            except Exception as e:
                _logger.error(f"Error generating cashflow for scenario_id={sid}: {e}")
                raise ValueError(
                    f"Failed to generate cashflow for scenario_id={sid}: {e}"
                )
        ...
        for row in cashflow_data:
            ...
            yearly_totals[year]["additional_income_net"] += row.get(
                "additional_income_net", 0
            )
            yearly_totals[year]["capital_return_net"] += row.get(
                "capital_return_net", 0
            )
