...
class DataService:
    ...
    def build_summary_table(
        client: Client, scenarios: List[Scenario], db: Session
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            case_name = (
                case_detection.case_name
                if hasattr(case_detection, "case_name")
                else "standard"
            )
            case_display_name = (
                case_detection.display_name
                if hasattr(case_detection, "display_name")
                else "מקרה רגיל"
            )
            case_reasons = (
                case_detection.reasons if hasattr(case_detection, "reasons") else []
            )
        ...
        summary = {
            "client_info": {
                ...
                "address": f"{client.address_street or ''}, {client.address_city or ''}".strip(
                    ", "
                )
                or "N/A",
            },
        ...
        try:
            employments = (
                db.query(Employment)
                .filter(Employment.client_id == client.id)
            ...
            for emp in employments:
                employer = (
                    db.query(Employer).filter(Employer.id == emp.employer_id).first()
                )
                emp_info = {
                    "employer_name": employer.name if employer else "N/A",
                    "start_date": (
                        emp.start_date.strftime("%d/%m/%Y") if emp.start_date else "N/A"
                    ),
                    "end_date": (
                        emp.end_date.strftime("%d/%m/%Y") if emp.end_date else "פעיל"
                    ),
                    "monthly_salary": (
                        f"{emp.monthly_salary_nominal:,.0f} ₪"
                        if emp.monthly_salary_nominal
                        else "N/A"
                    ),
        ...
        for scenario in scenarios:
            scenario_summary = {
                ...
                "has_results": bool(
                    scenario.summary_results and scenario.cashflow_projection
                ),
            }
            ...
            if scenario.summary_results:
                try:
                    results = (
                        ...
                        if isinstance(scenario.summary_results, str)
                        else scenario.summary_results
                    )
                    total_pension_val = (
            ...
            if scenario.cashflow_projection:
                try:
                    cashflow = (
                        json.loads(scenario.cashflow_projection)
                        if isinstance(scenario.cashflow_projection, str)
                        else scenario.cashflow_projection
                    )
