...
class StateService:
    ...
    def save_current_state(self) -> Dict:
        ...
        state = {
            ...
            "additional_incomes": [
                serialize_additional_income(ai) for ai in additional_incomes
            ],
            "termination_events": [
                serialize_termination_event(te) for te in termination_events
            ],
        }

        logger.info(
            f"  💾 Saved state: {len(pension_funds)} pension funds, {len(capital_assets)} capital assets"
        )
        return state
    ...
    def restore_state(self, state: Dict):
        ...
        self.db.query(PensionFund).filter(
            PensionFund.client_id == self.client_id
        ).delete()
        self.db.query(CapitalAsset).filter(
            CapitalAsset.client_id == self.client_id
        ).delete()
        self.db.query(AdditionalIncome).filter(
            AdditionalIncome.client_id == self.client_id
        ).delete()
        self.db.query(TerminationEvent).filter(
        ...
        self.db.flush()
        logger.info(
            f"  🔄 Restored state: {len(state['pension_funds'])} pension funds, {len(state['capital_assets'])} capital assets"
        )
