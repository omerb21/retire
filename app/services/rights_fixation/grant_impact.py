...
def compute_grant_effect(
    ...
    try:
        ...
        if isinstance(eligibility_date, str):
            eligibility_date_obj = datetime.strptime(
                eligibility_date, "%Y-%m-%d"
            ).date()
        else:
        ...
        # חישוב הפרש השנים
        years_diff = (
            (eligibility_date_obj.year - grant_date_obj.year)
        ...
        if years_diff > 15:
            logger.info(
                f"DEBUG: חוק 15 השנים חל - חלפו {years_diff:.2f} שנים בין המענק לזכאות"
            )
            ...
            logger.info(
                f"DEBUG: חוק 15 השנים לא חל - חלפו {years_diff:.2f} שנים בין המענק לזכאות"
            )
...
def compute_client_exemption(
    grants: List[Dict[str, Any]], eligibility_year: int
) -> Dict[str, Any]:
    """
