    ...
    try:
        ...
        try:
            ...
            if from_date > to_date_parsed:
                logger.warning(
                    f"תאריך התחלה {grant_date_str} מאוחר מתאריך סיום {to_date_str}"
                )
        ...
        logger.info(
            f"Calculated adjusted amount: {amount} from {grant_date_str} to {to_date_str} = {result}"
        )
        return result
        ...
        error_grant_date = (
            grant_date.isoformat() if isinstance(grant_date, date) else str(grant_date)
        )
        error_to_date = (
            to_date.isoformat()
            if to_date and isinstance(to_date, date)
            else (str(to_date) if to_date else "now")
        )
        logger.error(
            f"שגיאה בהצמדה מתאריך {error_grant_date} לתאריך {error_to_date}: {e}",
            exc_info=True,
        )
        return None
