...
class ScenarioService:
    @staticmethod
    def create_scenario(
        db: Session, client_id: int, data: ScenarioCreateIn
    ) -> Scenario:
        ...
        try:
            ...
            parameters = {
                "planned_termination_date": (
                    data.planned_termination_date.isoformat()
                    if data.planned_termination_date
                    else None
                ),
    ...
    def run_scenario(db: Session, scenario_id: int) -> ScenarioOut:
        ...
        try:
            ...
            if not employment:
                ...
                employment = (
                    db.query(Employment)
                    .filter(Employment.client_id == scenario.client_id)
            ...
            if parameters.get("planned_termination_date"):
                planned_termination_date = datetime.fromisoformat(
                    parameters["planned_termination_date"]
                ).date()

            # Determine termination date for calculation
            termination_date = (
                planned_termination_date
                or employment.planned_termination_date
                or date.today()
            )
            ...
            try:
                result = calc_engine.run(
                    client_id=scenario.client_id, scenario=scenario_in
                )
            except Exception as calc_error:
                if "׳¡׳“׳¨׳× ׳׳“׳“ ׳—׳¡׳¨׳”" in str(
                    calc_error
                ) or "׳׳™׳ ׳ ׳×׳•׳ ׳™ ׳×׳¢׳¡׳•׳§׳” ׳׳—׳™׳©׳•׳‘" in str(calc_error):
    ...
    def get_scenario(db: Session, scenario_id: int) -> Optional[ScenarioOut]:
        ...
        # Parse saved results
        summary_results = (
            json.loads(scenario.summary_results) if scenario.summary_results else {}
        )
        cashflow_data = (
    ...
    @staticmethod
    def _generate_cashflow_projection(
        result, monthly_expenses: float = 0
    ) -> List[CashflowPoint]:
        ...
        for month in range(12):
            ...
            month_date = date(
                base_date.year + (base_date.month + month - 1) // 12,
                ((base_date.month + month - 1) % 12) + 1,
                1,
            )
            ...
            cashflow.append(
                CashflowPoint(date=month_date, inflow=inflow, outflow=outflow, net=net)
            )
