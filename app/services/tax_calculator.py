...
class TaxCalculator:
    ...
    def __init__(self, tax_year: int = None):
        ...
        self.national_insurance = TaxConstants.get_national_insurance_rates(
            self.tax_year
        )
    ...
    def calculate_income_tax(
        self, taxable_income: float, is_business_income: bool = False
        ...
        for bracket in self.tax_brackets:
            ...
            if bracket.max_income:
                ...
                income_in_bracket = min(
                    ...
                    bracket.max_income
                    - processed_income,  # הגבול העליון של המדרגה פחות מה שכבר עיבדנו
                )
            else:
            ...
            if income_in_bracket > 0:
                ...
                logger.debug(
                    f"Tax Bracket {bracket.min_income}-{bracket.max_income}: "
                    f"Income {income_in_bracket:.2f} at {bracket.rate*100}% = {tax_in_bracket:.2f}"
                )

        logger.debug(
            f"Total Annual Tax: {total_tax:.2f} on income {taxable_income:.2f}"
        )
        return round(total_tax, 2), breakdown

    def calculate_national_insurance(
        self, annual_income: float, personal_details=None
    ) -> float:
        """
        מחשב ביטוח לאומי
    ...
    def calculate_applicable_credits(
        self, input_data: TaxCalculationInput
    ) -> Tuple[float, List[TaxCreditInput]]:
        """
    ...
    def calculate_pension_exemptions(
        self, pension_income: float, personal_details
    ) -> float:
        """
    ...
    def calculate_comprehensive_tax(
        ...
        if special_tax_incomes["rental_income"] > 0:
            rental_tax = (
                special_tax_incomes["rental_income"]
                * TaxConstants.SPECIAL_TAX_RATES["rental_income"]
            )
            ...
            logger.info(
                f"מס שכירות: {rental_tax:,.2f} על {special_tax_incomes['rental_income']:,.2f}"
            )

        # מס רווח הון - 25%
        if special_tax_incomes["capital_gains"] > 0:
            capital_gains_tax = (
                special_tax_incomes["capital_gains"]
                * TaxConstants.SPECIAL_TAX_RATES["capital_gains"]
            )
            ...
            logger.info(
                f"מס רווח הון: {capital_gains_tax:,.2f} על {special_tax_incomes['capital_gains']:,.2f}"
            )

        # מס דיבידנד - 25%
        if special_tax_incomes["dividend_income"] > 0:
            dividend_tax = (
                special_tax_incomes["dividend_income"]
                * TaxConstants.SPECIAL_TAX_RATES["dividend_income"]
            )
            ...
            logger.info(
                f"מס דיבידנד: {dividend_tax:,.2f} על {special_tax_incomes['dividend_income']:,.2f}"
            )

        # מס ריבית - 15%
        if special_tax_incomes["interest_income"] > 0:
            interest_tax = (
                special_tax_incomes["interest_income"]
                * TaxConstants.SPECIAL_TAX_RATES["interest_income"]
            )
            ...
            logger.info(
                f"מס ריבית: {interest_tax:,.2f} על {special_tax_incomes['interest_income']:,.2f}"
            )
        ...
        if input_data.pension_months_in_year < 12:
            ...
            adjusted_pension_income = (
                monthly_pension * input_data.pension_months_in_year
            )
            logger.info(
                f"התאמת הכנסת קצבה: {input_data.pension_income:,.2f} -> {adjusted_pension_income:,.2f} ({input_data.pension_months_in_year} חודשים)"
            )
        ...
        pension_exemption_regular = self.calculate_pension_exemptions(
            adjusted_pension_income, input_data.personal_details
        )
        ...
        logger.info(
            f'פטורי פנסיה: רגיל={pension_exemption_regular:,.2f}, מקיבוע זכויות={exempt_pension_annual:,.2f}, סה"כ={pension_exemption:,.2f}'
        )
        ...
        other_income = (
            taxable_income - business_income if taxable_income > business_income else 0
        )

        # חישוב מס על הכנסה מעסק
        business_tax, business_breakdown = self.calculate_income_tax(
            business_income, is_business_income=True
        )

        # חישוב מס על שאר ההכנסות
        other_tax, other_breakdown = self.calculate_income_tax(
            other_income, is_business_income=False
        )
        ...
        national_insurance = self.calculate_national_insurance(
            regular_taxable_income, input_data.personal_details
        )
        health_tax = self.calculate_health_tax(
            regular_taxable_income, input_data.personal_details
        )

        # 9. חישוב זיכויים
        tax_credits_amount, applied_credits = self.calculate_applicable_credits(
            input_data
        )

        # 10. סך המסים
        total_tax_before_credits = (
            income_tax + national_insurance + health_tax + total_special_tax
        )
        ...
        logger.info(
            f"פירוט: מס הכנסה={income_tax:,.2f}, בל={national_insurance:,.2f}, בריאות={health_tax:,.2f}, מיוחדים={total_special_tax:,.2f}"
        )
        return result

    def generate_optimization_suggestions(
        self, input_data: TaxCalculationInput, current_result: TaxCalculationResult
        ...
        if input_data.pension_contributions < input_data.salary_income * 0.07:
            ...
            additional_contribution = (
                max_contribution - input_data.pension_contributions
            )
            potential_savings = (
                additional_contribution * current_result.marginal_tax_rate / 100
            )
        ...
        if input_data.study_fund_contributions == 0 and input_data.salary_income > 0:
            max_study_fund = min(
                input_data.salary_income * 0.075, 8500
            )  # מקסימום לשנת 2024
        ...
        # הצעה לתרומות
        if (
            input_data.charitable_donations
            < input_data.get_total_annual_income() * 0.35
        ):
            max_donation = input_data.get_total_annual_income() * 0.35
            suggested_donation = min(10000, max_donation)  # הצעה סבירה
            potential_savings = (
                suggested_donation * current_result.marginal_tax_rate / 100
            )
