...
class ReportServiceAdapter:
    ...
    def normalize_pdf_request(
        scenario_id: Optional[int] = None, request: Optional[ReportPdfRequest] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "sections": (
                request.sections
                if request and hasattr(request, "sections")
                else {
                    "summary": True,
                    "cashflow_table": True,
                    "net_chart": True,
    ...
    def generate_pdf_report(
        db: Session, client_id: int, scenario_id: int, request: ReportPdfRequest
    ) -> bytes:
        """
        ...
        try:
            ...
            normalized = ReportServiceAdapter.normalize_pdf_request(
                scenario_id, request
            )
            logger.info(
                f"Normalized PDF request: client_id={client_id}, scenario_ids={normalized['scenario_ids']}"
            )
            ...
            return generate_report_pdf(
                db=db,
                client_id=client_id,
                scenario_id=normalized["scenario_ids"][
                    0
                ],  # Use first scenario for single-scenario PDF
                request=request,
            )
...
class CompareServiceAdapter:
    ...
    def compare_scenarios_for_api(
        ...
        for scenario_id, scenario_data in data.get("scenarios", {}).items():
            ...
            for year in yearly_data:
                ...
                additional_income = Decimal(
                    str(yearly_data[year]["additional_income_net"])
                )
                ...
                yearly_data[year]["net"] = float(
                    net_calculated.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                )
        ...
        return {"scenarios": scenarios_list, "meta": data.get("meta", {})}
...
class CaseDetectionAdapter:
    ...
    def detect_case_for_api(db_session: Session, client_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "case": (
                    result.case_name if hasattr(result, "case_name") else "standard"
                ),
                "case_id": result.case_id if hasattr(result, "case_id") else 1,
            }
        except Exception:
            # Fallback to standard case
            return {"case": "standard", "case_id": 1}
