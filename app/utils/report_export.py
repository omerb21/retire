...
def export_pdf(
    client_id: int, scenarios: List[Dict[str, Any]], path: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # Title
        title = Paragraph(
            f"Retirement Planning Report - Client {client_id}", title_style
        )
        story.append(title)
        ...
        for i, scenario in enumerate(scenarios, 1):
            scenario_heading = Paragraph(
                f"Scenario {i}: {scenario.get('scenario_name', 'Unnamed')}",
                heading_style,
            )
            ...
            params_text += (
                f"Capitalization: {scenario.get('apply_capitalization', False)}, "
            )
            params_text += (
                f"Exemption Shield: {scenario.get('apply_exemption_shield', False)}"
            )
            ...
            if cashflow_data:
                ...
                if cashflow_data and "monthly" in cashflow_data:
                    ...
                    cashflow_heading = Paragraph(
                        "Monthly Cashflow Projection", styles["Heading3"]
                    )
                    story.append(cashflow_heading)

                    # Create table data
                    table_data = [
                        ["Month", "Date", "Income", "Expenses", "Net", "Cumulative"]
                    ]

                    for month_data in cashflow_data["monthly"][
                        :6
                    ]:  # Show first 6 months
                        table_data.append(
                            [
                    ...
                    if yearly_totals:
                        ...
                        yearly_data = [
                            ["Year", "Total Income", "Total Expenses", "Net Total"]
                        ]
