          ...
          {result && (
            <>
              ...
              <div style={{ marginBottom: '20px', padding: '15px', border: '1px solid #28a745', borderRadius: '4px', backgroundColor: '#f8fff9' }}>
                <h3>סיכום כללי</h3>
                <div><strong>סך הכנסה:</strong> ₪{result.total_income.toLocaleString()}</div>
                <div><strong>הכנסה חייבת:</strong> ₪{result.taxable_income.toLocaleString()}</div>
                <div><strong>הכנסה פטורה:</strong> ₪{result.exempt_income.toLocaleString()}</div>
                <div style={{ color: '#dc3545' }}><strong>מס נטו לתשלום:</strong> ₪{result.net_tax.toLocaleString()}</div>
                <div style={{ color: '#28a745' }}><strong>הכנסה נטו:</strong> ₪{result.net_income.toLocaleString()}</div>
              ...
              <div style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ddd', borderRadius: '4px' }}>
                <h3>פירוט מסים</h3>
                <div><strong>מס הכנסה:</strong> ₪{result.income_tax.toLocaleString()}</div>
                <div><strong>ביטוח לאומי:</strong> ₪{result.national_insurance.toLocaleString()}</div>
                <div><strong>מס בריאות:</strong> ₪{result.health_tax.toLocaleString()}</div>
                <div><strong>סך מסים:</strong> ₪{result.total_tax.toLocaleString()}</div>
                <div style={{ color: '#28a745' }}><strong>זיכויים:</strong> ₪{result.tax_credits_amount.toLocaleString()}</div>
              </div>
              ...
              {result.applied_credits.length > 0 && (
                <div style={{ marginBottom: '20px', padding: '15px', border: '1px solid #17a2b8', borderRadius: '4px', backgroundColor: '#f0f9ff' }}>
                  ...
                  {result.applied_credits.map((credit, index) => (
                    <div key={index}>
                      <strong>{credit.description}:</strong> ₪{credit.amount.toLocaleString()}
                    </div>
                  ))}
                </div>
              )}
                      ...
                      {result.tax_breakdown.map((bracket, index) => (
                        <tr key={index}>
                          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                            ₪{bracket.bracket_min.toLocaleString()} - {bracket.bracket_max ? `₪${bracket.bracket_max.toLocaleString()}` : 'ומעלה'}
                          </td>
                          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                            {(bracket.rate * 100).toFixed(1)}%
                          </td>
                          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                            ₪{bracket.taxable_amount.toLocaleString()}
                          </td>
                          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                            ₪{bracket.tax_amount.toLocaleString()}
                          </td>
                        </tr>
                      ))}
                    </tbody>
