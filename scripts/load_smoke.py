...
class LoadTester:
    ...
    def test_cashflow_generate(
        self, client_id: int = 1, scenario_id: int = 24
    ) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                ...
                json={"from": "2025-01", "to": "2025-12", "frequency": "monthly"},
                timeout=5.0,
            )
    ...
    def test_scenario_compare(
        self, client_id: int = 1, scenarios: List[int] = [24, 25]
    ) -> Dict[str, Any]:
    ...
    def run_load_test(self):
        ...
        try:
            ...
            if health_response.status_code != 200:
                print(
                    f"❌ Server not responding properly: {health_response.status_code}"
                )
                return
        except Exception as e:
    ...
    def analyze_results(
        ...
        if success_rate < 100:
            ...
            for failed in failed_tests[:5]:  # Show first 5 failures
                print(
                    f"  - {failed['endpoint']}: {failed.get('error', 'HTTP ' + str(failed['status_code']))}"
                )
        else:
        ...
        for endpoint_name, results in endpoints.items():
            ...
            if successful_results:
                ...
                print(
                    f"{endpoint_name:<20} {len(results):<8} {avg_duration:<10.0f} {median_duration:<12.0f} {p95_duration:<10.0f} {status}"
                )
            else:
                print(
                    f"{endpoint_name:<20} {len(results):<8} {'FAILED':<10} {'FAILED':<12} {'FAILED':<10} ❌"
                )
        ...
        cashflow_median = (
            statistics.median(
                [r["duration_ms"] for r in cashflow_results if r["success"]]
            )
            if successful_cashflow > 0
            else 0
        )
        compare_median = (
            statistics.median(
                [r["duration_ms"] for r in compare_results if r["success"]]
            )
            if successful_compare > 0
            else 0
        )
        pdf_median = (
            statistics.median([r["duration_ms"] for r in pdf_results if r["success"]])
            if successful_pdf > 0
            else 0
        )

        print(
            f"Success: {success_rate:.1f}% | Cashflow: {cashflow_median:.0f}ms | Compare: {compare_median:.0f}ms | PDF: {pdf_median:.0f}ms"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run load tests for retirement planning API"
    )
    parser.add_argument(
        "--url", default="http://127.0.0.1:8002", help="Base URL for API"
    )
    parser.add_argument(
        "--quick", action="store_true", help="Run quick test (10 calls each)"
    )
