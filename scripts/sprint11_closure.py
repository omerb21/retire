...
try:
    ...
    if case_response.ok:
        ...
        print(
            f"CASE DETECT - STATUS {case_response.status_code} + case={case_data['result']['case_id']} ({case_data['result'].get('case_name', '')})"
        )
...
try:
    ...
    if cashflow_response.ok:
        ...
        min_month = (
            min(item.get("month", "N/A") for item in cashflow_data)
            if cashflow_data
            else "N/A"
        )
        max_month = (
            max(item.get("month", "N/A") for item in cashflow_data)
            if cashflow_data
            else "N/A"
        )
...
try:
    ...
    compare_response = requests.post(
        compare_url, json=compare_body, headers={"Content-Type": "application/json"}
    )
    ...
    if compare_response.ok:
        ...
        for scenario, totals in yearly_totals.items():
            print(
                f"  Scenario {scenario}: Income={totals.get('income', 0):.1f}k, "
