...
def main():
    ...
    print(
        f"4) PDF: STATUS {pdf_r.status_code}  size={len(pdf_bytes)}  %PDF={'OK' if pdf_ok else 'NO'}  "
    ...
    print(
        f"ZIP: {os.path.basename(zip_path)} -> {os.path.getsize(zip_path)} bytes -> {sha256_file(zip_path)}"
    )
    print("=== END SUMMARY ===")
