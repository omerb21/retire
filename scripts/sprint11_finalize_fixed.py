...
def example_for(prop):
    ...
    if t == "boolean":
        return False
    if t == "array":
        return [example_for(prop.get("items", {}))]
    if t == "object":
        res = {}
...
def main():
    ...
    if case_path:
        ...
        content = ((post.get("requestBody") or {}).get("content") or {}).get(
            "application/json"
        )
    ...
    content_lines.append(
        "REQUIRED: " + (", ".join(req_fields) if req_fields else "(none)")
    )
    ...
    # URLs
    cashflow_url = (
        f"{base}/api/v1/scenarios/{SCENARIO_ID}/cashflow/generate?client_id={CLIENT_ID}"
    )
    compare_url = f"{base}/api/v1/clients/{CLIENT_ID}/scenarios/compare"
    report_url = (
        f"{base}/api/v1/scenarios/{SCENARIO_ID}/report/pdf?client_id={CLIENT_ID}"
    )
    ...
    try:
        ...
        save_json(
            os.path.join(
                RUN_DIR, "case_detect_200.json" if r.ok else "case_detect_error.json"
            ),
    ...
    save_json(
        os.path.join(
            RUN_DIR, "cashflow_200.json" if cf_r.ok else "cashflow_error.json"
        ),
    ...
    if cmp_r.headers.get("content-type", "").startswith("application/json"):
        cmp_json = cmp_r.json()
    save_json(
        os.path.join(RUN_DIR, "compare_200.json" if cmp_r.ok else "compare_error.json"),
    ...
    if isinstance(cmp_json, dict) and "results" in cmp_json:
        for item in cmp_json["results"]:
            if item.get("scenario_id") == SCENARIO_ID and isinstance(
                item.get("monthly"), list
            ):
    ...
    with open(
        os.path.join(RUN_DIR, "report_ok.pdf" if pdf_ok else "report_error.bin"), "wb"
    ) as f:
    ...
    print(
        f"1) CASE DETECT: STATUS {case_status}  case={case_json.get('case') if isinstance(case_json, dict) else '(n/a)'}"
    )
    print(
        f"2) CASHFLOW: STATUS {cf_status}  rows={len(cf_rows) if isinstance(cf_rows, list) else 0} (expect 12)"
    )
    y2025 = totals.get("2025")
    if y2025:
        print(
            f"3) COMPARE (2025 totals): inflow={y2025['inflow']:,} outflow={y2025['outflow']:,} add_income={y2025['additional_income_net']:,} cap_return={y2025['capital_return_net']:,} net={y2025['net']:,}"
        )
    else:
        print("3) COMPARE: yearly totals unavailable")
    print(
        f"4) PDF: STATUS {pdf_r.status_code}  size={len(pdf_r.content or b'')}  %PDF={'OK' if pdf_ok else 'NO'}"
    )
