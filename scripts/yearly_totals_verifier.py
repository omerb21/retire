...
class YearlyTotalsVerifier:
    ...
    def validate_monthly_data(
        self, scenario_id: str, monthly_data: List[Dict]
    ) -> List[Dict]:
        ...
        for row in monthly_data:
            ...
            for field in [
                "inflow",
                "outflow",
                "additional_income_net",
                "capital_return_net",
                "net",
            ]:
                value = row.get(field)
                if not isinstance(value, (int, float, Decimal)) or (
        ...
        if monthly_data:
            ...
            if self.validate_date_format(start_date) and self.validate_date_format(
                end_date
            ):
    ...
    def validate_yearly_totals(
        self, scenario_id: str, computed: Dict[int, Dict], reported: Dict[str, Dict]
    ) -> List[Dict]:
    ...
    def validate_scenario(
        self, scenario_id: str, monthly_data: List[Dict], yearly_totals: Dict[str, Dict]
    ) -> Dict:
    ...
    def set_api_status(self, endpoint: str, status_code: int, **details) -> None:
        ...
        self.api_status[endpoint] = {"status_code": status_code, **details}
    ...
    def create_zip_artifact(self) -> Dict:
        ...
        return {"path": self.zip_path, "hash": zip_hash, "size": zip_size}
    ...
    def generate_proof_summary(self) -> str:
        ...
        for filename, details in self.artifacts.items():
            summary.append(
                f"- {filename:<25} SHA256={details['hash']}  bytes={details['size']}"
            )
        ...
        for scenario_id, result in self.validation_results.items():
            ...
            for year, year_result in result["years"].items():
                summary.append(
                    f"  {year}: months_expected={year_result['months_expected']} "
                ...
                for field in ["inflow", "outflow", "add_income", "cap_return", "net"]:
                    ...
                    if (
                        source_field in year_result["reported"]
                        and source_field in year_result["computed"]
                    ):
                        summary.append(
                            f"        {display_field}: reported={year_result['reported'][source_field]} "
        ...
        # Add ZIP info
        summary.append(
            f"ZIP: {zip_details['path']}  SHA256={zip_details['hash']}  bytes={zip_details['size']}"
        )
    ...
    def verify_data(self, data: Dict) -> str:
        ...
        for scenario in data.get("scenarios", []):
            ...
            self.add_artifact(
                f"cashflow_{scenario_id}.json", json.dumps(monthly_data, indent=2)
            )

            # Add yearly totals as artifact
            self.add_artifact(
                f"yearly_totals_{scenario_id}.json", json.dumps(yearly_totals, indent=2)
            )
...
def main():
    ...
    example_data = {
        ...
        "artifacts": {"report_ok.pdf": b"%PDF-1.4\n...sample PDF content..."},
    }
