...
def test_stage16_determinism_report(monkeypatch) -> None:
    ...
    for c in (action.get("cases") or []):
        if isinstance(c, dict):
            cases.append({"case_id": f"action::{c.get('case_id')}", "user_text": str(c.get("user_text") or "")})

    for i in (overlap.get("inputs") or []):
        if isinstance(i, dict):
            cases.append({"case_id": f"overlap::{i.get('input_id')}", "user_text": str(i.get("user_text") or "")})

    for q in (qa.get("questions") or []):
        if isinstance(q, dict):
            cases.append({"case_id": f"qa::{q.get('question_id')}", "user_text": str(q.get("question_text") or "")})
