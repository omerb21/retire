...
def test_stageF_stream_executes_only_when_outcome_final_tool_allowed(
    ...
    with Session() as db:
        res = user_approved_exec._maybe_handle_user_approved_json_exec(
            request=request,
            db=db,
            stream_request_id="trace_stageF_stream_outcome_only",
    ...
    assert tool_calls
    assert "🔧" in body
    assert "אין בקשת אישור פתוחה תואמת" not in body
...
def test_stageF_stream_blocks_when_outcome_final_tool_blocked(monkeypatch, _test_db) -> None:
    ...
    with Session() as db:
        res = user_approved_exec._maybe_handle_user_approved_json_exec(
            request=request,
            db=db,
            stream_request_id="trace_stageF_stream_outcome_only_block",
            original_user_msg=payload,
        )
