...
def test_execute_tool_call_dispatches_retirement_scenario_tools(
    ...
    def _stub_run(*, args, agent_tools) -> str:
        ...
        return json.dumps(
            {"stub": True, "tool": "RUN_RETIREMENT_SCENARIOS"}, ensure_ascii=False
        )
    ...
    def _stub_select(*, args, agent_tools) -> str:
        ...
        assert agent_tools is not None
        return json.dumps(
            {"stub": True, "tool": "SELECT_TARGET_PENSION_SCENARIO"}, ensure_ascii=False
        )
    ...
    def _stub_find(*, args, agent_tools) -> str:
        ...
        assert agent_tools is not None
        return json.dumps(
            {"stub": True, "tool": "FIND_OPTIMAL_SCENARIO"}, ensure_ascii=False
        )
    ...
    def _stub_execute(*, args, client_id: int, db) -> str:
        ...
        return json.dumps(
            {"stub": True, "tool": "EXECUTE_RETIREMENT_SCENARIO"}, ensure_ascii=False
        )

    monkeypatch.setattr(tool_execution, "handle_run_retirement_scenarios", _stub_run)
    monkeypatch.setattr(
        tool_execution, "handle_select_target_pension_scenario", _stub_select
    )
    monkeypatch.setattr(tool_execution, "handle_find_optimal_scenario", _stub_find)
    monkeypatch.setattr(
        tool_execution, "handle_execute_retirement_scenario", _stub_execute
    )
