...
def test_stream_injects_base_prompt_and_playbook_into_llm_messages_no_tools(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        stream_loop,
        "_load_stream_intents_playbook_text",
        lambda: "PLAYBOOK_INTENTS_TEST",
    )
    ...
    assert any(
        "NO_TOOLS" in c and "REPORT" in c and "ANALYSIS" in c for c in system_contents
    )
    ...
    playbook_idx = next(
        i for i, c in enumerate(system_contents) if "PLAYBOOK_INTENTS_TEST" in c
    )
    intent_idx = next(i for i, c in enumerate(system_contents) if "מצב: NO_TOOLS" in c)
    assert base_idx < playbook_idx < intent_idx


def test_stream_injects_base_prompt_and_playbook_into_llm_messages_analysis(
    monkeypatch,
) -> None:
    captured = {"messages": None}
