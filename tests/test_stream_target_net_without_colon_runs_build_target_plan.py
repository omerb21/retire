...
def test_stream_target_net_without_colon_runs_build_target_plan(monkeypatch, _test_db) -> None:
    Session = _test_db["Session"]

    with Session() as db:
        client = db.query(Client).filter(Client.id == 940000003).first()
        if client is None:
            client = Client(
                id=940000003,
                id_number_raw="940000003",
                id_number="940000003",
                full_name="Test User",
    ...
    def fake_chat_stream(messages, client_id=None):
        raise AssertionError("LLM must not be called for pending_plan_target numeric continue triggers tools-first")
    ...
    resp1 = api.post(
        ...
        json={
            "client_id": client_id,
            "messages": [{"role": "user", "content": "לבנות תכנית פרישה"}],
            "pension_portfolio": [],
        },
    )
