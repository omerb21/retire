...
def get_current_tax_parameters_snapshot() -> str:
    ...
    try:
        ...
        latest_params = (
            db.query(TaxParameters).order_by(TaxParameters.valid_from.desc()).first()
        )
        ...
        if latest_params:
            ...
            default_params = {
                ...
                "exemption_amounts": {"personal": 2640, "spouse": 2640, "child": 2640},
            }
...
def run_from_log(trace_id: str) -> Dict[str, Any]:
    ...
    try:
        # Get log entry
        log_entry = (
            db.query(CalculationLog).filter(CalculationLog.trace_id == trace_id).first()
        )
        ...
        return {
            ...
            "original_output": (
                json.loads(log_entry.output_snapshot)
                if log_entry.output_snapshot
                else None
            ),
...
def get_calculation_history(
    client_id: int = None, function_name: str = None, limit: int = 100
) -> List[Dict[str, Any]]:
    """
