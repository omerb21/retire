...
class RetirementReportBuilder:
    ...
    def build_pdf(
        self, context: Dict[str, Any], cashflow: List[Dict[str, Any]], file_path: str
    ) -> str:
        """
    ...
    def _build_executive_summary(
        self, context: Dict[str, Any], cashflow: List[Dict[str, Any]]
    ) -> List:
        ...
        if cashflow:
            ...
            total_gross = total_net = total_tax = avg_annual_net = (
                effective_tax_rate
            ) = 0
        ...
        if cashflow:
            ...
            insights.append(
                f"• שנת ההכנסה הגבוהה ביותר: {peak_year_data['year']} ({peak_year_data['net_income']:,.0f} ש\"ח)"
            )

            # Tax burden analysis
            if effective_tax_rate > 25:
                insights.append(
                    f"• נטל המס גבוה יחסית ({effective_tax_rate:.1f}%) - מומלץ לבחון אפשרויות אופטימיזציה"
                )
            else:
            ...
            if total_pension > total_grants:
                insights.append(
                    f'• הכנסה מקצבאות ({total_pension:,.0f} ש"ח) גבוהה מהכנסה ממענקים ({total_grants:,.0f} ש"ח)'
                )
            else:
                insights.append(
                    f'• הכנסה ממענקים ({total_grants:,.0f} ש"ח) גבוהה מהכנסה מקצבאות ({total_pension:,.0f} ש"ח)'
                )
    ...
    def _build_detailed_analysis(
        ...
        if processed_grants:
            ...
            grant_table = Table(
                grant_data, colWidths=[3 * cm, 3 * cm, 3 * cm, 2.5 * cm]
            )
    ...
    def _build_charts_section(self, cashflow: List[Dict[str, Any]]) -> List:
        ...
        # Section title
        elements.append(
            Paragraph("גרפים וניתוחים חזותיים", self.styles["CustomSubtitle"])
        )

        if not cashflow:
            elements.append(
                Paragraph("אין נתוני תזרים זמינים להצגה", self.styles["Normal"])
            )
            return elements
        ...
        try:
            ...
            elements.append(
                Paragraph("תזרים מזומנים לאורך זמן", self.styles["SectionHeader"])
            )
            elements.append(Image(cashflow_chart, width=6 * inch, height=4 * inch))
            elements.append(Spacer(1, 0.2 * inch))
        except Exception as e:
            elements.append(
                Paragraph(f"שגיאה ביצירת גרף תזרים: {str(e)}", self.styles["Normal"])
            )

        # Income breakdown chart
        try:
            breakdown_chart = render_income_breakdown_chart(
                cashflow, "פירוט הכנסות לפי מקור"
            )
            elements.append(
                Paragraph("פירוט הכנסות לפי מקור", self.styles["SectionHeader"])
            )
            elements.append(Image(breakdown_chart, width=6 * inch, height=4 * inch))
            elements.append(Spacer(1, 0.2 * inch))
        except Exception as e:
            elements.append(
                Paragraph(f"שגיאה ביצירת גרף פירוט: {str(e)}", self.styles["Normal"])
            )
        ...
        try:
            ...
            elements.append(
                Paragraph(f"שגיאה ביצירת גרף מצטבר: {str(e)}", self.styles["Normal"])
            )
    ...
    def _build_cashflow_tables(self, cashflow: List[Dict[str, Any]]) -> List:
        ...
        # Section title
        elements.append(
            Paragraph("טבלאות תזרים מפורטות", self.styles["CustomSubtitle"])
        )
        ...
        for i in range(0, len(cashflow), chunk_size):
            ...
            table = Table(
                table_data,
                colWidths=[1.5 * cm, 2.5 * cm, 2 * cm, 2 * cm, 2 * cm, 2.5 * cm],
            )
            table.setStyle(
                TableStyle(
                    [
            ...
            for i, bracket in enumerate(brackets):
                tax_data.append(
                    [
                        ...
                        (
                            f"{bracket.get('max', 'ללא הגבלה'):,.0f}"
                            if isinstance(bracket.get("max"), (int, float))
                            else "ללא הגבלה"
                        ),
    ...
    def _add_header_footer(self, canvas, doc):
        ...
        canvas.drawString(
            2 * cm, 1 * cm, f"נוצר ב-{datetime.now().strftime('%d/%m/%Y %H:%M')}"
        )
...
def build_pdf(
    context: Dict[str, Any], cashflow: List[Dict[str, Any]], file_path: str
) -> str:
    """
